...
open class Engine {
    ...
    func setPref(_ prefName: String, prefValue: Any) {
        self.getBridge().callAction("core:setPref", args: [prefName as AnyObject, prefValue as! NSObject])
    }
    
    func getPref(_ prefName: String) -> Any {
        let result = self.getBridge().callAction("core:getPref", args: [prefName as AnyObject])
