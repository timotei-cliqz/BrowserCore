...
open class WebViewProgress
{
    ...
    class LoadingObserver : NSObject {
        fileprivate weak var webView: CustomWebView?
        ...
        @objc override func observeValue(forKeyPath keyPath: String?, of object: Any?, change: [NSKeyValueChangeKey : Any]?, context: UnsafeMutableRawPointer?) {
            ...
            DispatchQueue.main.async { // ensure closure is on main thread, by-definition this func can be off-main
...
//            }
    ...
    open func shouldStartLoadWithRequest(_ request: URLRequest, navigationType: UIWebViewNavigationType) -> Bool {
        ...
        let isHTTPOrLocalFile = false //(request.url?.scheme?.startsWith("http") ?? false) || (request.url?.scheme?.startsWith("file") ?? false)
